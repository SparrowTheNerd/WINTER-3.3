...
ADXL375::ADXL375(I2C_HandleTypeDef *hi2c, uint8_t ODR, int8_t ofset[3]) {
    ...
    this->accel_ms2[0] = 0.0f;
    this->accel_ms2[1] = 0.0f;
...
HAL_StatusTypeDef ADXL375::Init() {
    ...
    HAL_StatusTypeDef status = ReadReg(ADXL375_REG_DEVID, &regDat, 1);
    if (status != HAL_OK) {
        return status;
    }
 ...
 * @retval HAL Status
 */
HAL_StatusTypeDef ADXL375::ReadAccel() {
    uint8_t data[6];
    HAL_StatusTypeDef status = ReadReg(ADXL375_REG_DATAX0, data, 6);
    if (status != HAL_OK) {
        return status;
    }
    ...
    accel_ms2[0] = (float)x * ADXL375_CONVERT; 
    accel_ms2[1] = (float)y * ADXL375_CONVERT;
    accel_ms2[2] = (float)z * ADXL375_CONVERT;
...
/**
  ...
  * @retval HAL Status
  */
HAL_StatusTypeDef ADXL375::ReadReg(uint8_t reg, uint8_t *data, uint8_t len) {
    return HAL_I2C_Mem_Read(hi2c, ADXL375_ADDR, reg, I2C_MEMADD_SIZE_8BIT, data, len, HAL_MAX_DELAY);
}
...
/**
  ...
  * @retval HAL Status
  */
HAL_StatusTypeDef ADXL375::WriteReg(uint8_t reg, uint8_t *data) {
    return HAL_I2C_Mem_Write(hi2c, ADXL375_ADDR, reg, I2C_MEMADD_SIZE_8BIT, data, 1, HAL_MAX_DELAY);
}